    ...
    class DB
    {
       ...
       public static function setTable($tableName)
       {
        ...
        public function find()
        {
            ...
            $field = $this->deField();
            $sql = "SELECT {$field} FROM " . self::$tableName . "{$where}" ."{$order}"." LIMIT 1";
            //添加调试SQL信息
            DEBUG::addMsg($sql,2);
            $arr = $this->query($sql);
            if ( empty($arr) ) {
        ...
        public function select()
        {
            ...
            $limit = $this->deLimit();
            $field = $this->deField();
            $sql = "SELECT {$field} FROM " . self::$tableName . "{$where}" ."{$order}"."{$limit}";
            //添加调试SQL信息
            DEBUG::addMsg($sql,2);
            $arr = $this->query($sql);
            return $arr;
        ...
        public function insert($arr = null)
        {
            ...
            $sql = "INSERT INTO `".self::$tableName."`(".$str1.")values(".$str2.")";
            //返回添加后的ID
            if ( mysql_query($sql) ) {
        ...
        public function update($arr = null)
        {
            ...
            $where = $this->deWhere();
            $sql = "UPDATE `".self::$tableName."` SET ".substr($sql, 0, -1)." WHERE {$where}";
            if ( mysql_query($sql) ) {
                return true;
            } else {
                DEBUG::addMsg($sql);
        ...
        {
            $sql = "SHOW COLUMNS FROM ".self::$tableName;
            DEBUG::addMsg($sql,2);
            $res = mysql_query($sql);
            $field = array();
            while ($row = mysql_fetch_assoc($res)) {
